...
REDIS_INCLUDE_PATH = /usr/include/hiredis

CFLAGS=-W -I$(INCLUDE_PATH) -I$(MYSQL_PLUS_PLUS_INCLUDE_PATH) -I$(MYSQL_INCLUDE_PATH) -I$(BIN_PATH) -I$(TINYXML_INCLUDE_PATH) \
		 -I$(REDIS_INCLUDE_PATH) -Icommon -Ibase -Icell -Itest -Icwmd -Ilogin -Idbmgr -Itimerd  -c -g -std=c++0x -DSYNC_DB_TEST -D__LUA_5_2_1 -D_USE_REDIS -D__AOI_PRUNING \
		 -D__SPEED_CHECK  \
		 -D__PLUTO_ORDER \
		 -D__RELOGIN \
		 -D__PLAT_PLUG_IN_NEW \
...
LFLAGS=-L$(LIB_PATH) -L$(MYSQL_LIB_PATH) -L$(LOCAL_LIB_PATH) -L$(TINYXML_LIB_PATH) \
	-g -llua5.2 -ltinyxml -ldl -lm  -lpthread -lmysqlcppconn -lmysqlclient -luuid -lhiredis -lcurl -lboost_regex -Wl,-E
		#-llua -ltinyxml -ldl -lm  -g -pg -lpthread -lmysqlclient -luuid -lhiredis -lcurl -Wl,-E
LUALIB=$(TINYXML_LIB_PATH)/liblua5.2.a $(TINYXML_LIB_PATH)/libtinyxml.a $(TINYXML_LIB_PATH)/libssl.a $(TINYXML_LIB_PATH)/libcrypto.a \
       $(TINYXML_LIB_PATH)/libboost_regex.a #$(LOCAL_LIB_PATH)/libmsgpack.a
